...
import EditTicketStaffForm from '../components/EditTicketStaffForm'
...
const TicketListPage = (props) => {
  const { actions, listAllTickets, tickets, profile } = props
  ...
  useEffect(() => {
    if (!raisedTickets?.length) {
      const url = 'ticketsStaff/all'
      const userEmail = profile?.email
      if (userEmail) {
        listAllTickets(url, userEmail)
      }
    }
  }, [profile])
